...
Assume $k$ to be stationary and isotropic on $\mathcal{U}=\mathbb{R}^{d_x}$, \textit{i.e.} $k(x,x')=k(\|x-x'\|)$. By Bochner's theorem, $k$ is then the inverse Fourier transform of a non-negative spectral density $S$, a duality known as the Wiener-Khinchin theorem:
\begin{equation}
