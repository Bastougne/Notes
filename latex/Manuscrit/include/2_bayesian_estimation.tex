...
\begin{equation}
	\underset{\psi\in\mathcal{F}}{\textup{argmin}}(\rho(\psi(\mathbf{z}_{1:k}),\varphi(x_k)))\label{eq:loss_function_with_k}
\end{equation}

Depending on the considered space and distance, different optimal estimators arise. We present three classical ones below. For the sake of simplicity, we write $Z$ for a generic observation vector and $X$ for the underlying state. At a fixed time step $k$, \eqref{eq:loss_function_with_k} thus becomes:
\begin{equation}
