...
While they alleviate, to a degree, the limitations induced by \eqref{eq:Kalman_assumptions}, Kalman-based methods all enforce the PDF to be Gaussian at each time step. This assumption is often violated in practice, as navigation problems frequently involve highly non-linear and non-injective functions. In turn, these functions induce multimodal or heavy-tailed distributions which cannot be adequately characterised by Gaussian hypotheses.

Monte Carlo (MC) methods offer an alternative approach that remains consistent under very mild assumptions. Their accuracy improves asymptotically with the number of samples, at the expense of an increased computational cost. This section introduces the main concepts for MC-based algorithms to solve the navigation problem \eqref{eq:navigation_problem}.
...
\end{equation}
converges a.s. to $1$, but each finite sample is not necessarily normalised. Furthermore, $p$ and $q$ are often known only up to normalising constants $\tilde{p}=C_\mu p$ and $\tilde{q}=C_\nu q$. To alleviate these issues, \Cref{th:self_normalised_measure} introduces the self-normalised measure, whose estimator remains consistent despite these unknown constants, as proven by \Cref{th:self_normalised_estimator}.
...
While the current formulation allows us to approximate the expectation of the full trajectory $p(\mathbf{x}_{0:k}|\mathbf{z}_{1:k})$, we often only want to estimate the current state $x_k$. This can be done naturally by letting $\pi_k$ denote the projection of a $(k+1)$-tuple onto its last element:
\begin{equation}
	\pi_k:\left\{\begin{aligned}
	\left(\mathbb{R}^{d_x}\right)^{k+1}&\rightarrow\mathbb{R}^{d_x}\\
	(x_0,\dots,x_k)&\mapsto x_k
	\end{aligned}\right.
\end{equation}
and computing the self-normalised estimator for $\varphi=\pi_k$:
\begin{equation}
...
\begin{equation}
	\hat{P}_k=\sum_{i=1}^Nw_k^{(i)}\left(x_k^{(i)}-\hat{x}_k\right)\left(x_k^{(i)}-\hat{x}_k\right)^\top
\end{equation}
...
\begin{algorithm}[H]
	...
	\State{$\displaystyle\hat{x}_k=\sum_{i=1}^Nw_k^{(i)}\:x_k^{(i)}$}\Comment{State estimation}
	\State{$\displaystyle\hat{P}_k=\sum_{i=1}^Nw_k^{(i)}\left(x_k^{(i)}-\hat{x}_k\right)\left(x_k^{(i)}-\hat{x}_k\right)^\top$}\Comment{State covariance}
...
\begin{remark}[Feynman-Kac distribution]
	...
	\end{equation}
	to show that the PDF is a Feynman-Kac distribution, with the transitions $p(x_j|x_{j-1})$ acting as the mutation and the likelihoods $p(z_j|x_j)$ as the selection function. Rather than being an artifice used to cancel the unknown constants, the self-norma\-lisation introduced in \Cref{th:self_normalised_measure} is thus, more fundamentally, the normalisation of this distribution.
\end{remark}
...
\begin{proposition}[Weight degeneracy]\label{th:variance_weights}
	Let $\tilde{w}_{0:k}$ denote the non-normalised weight associated with the entire trajectory:
	\begin{equation}
		\tilde{w}_{0:k}\propto\frac{\tilde{p}(\mathbf{x}_{0:k}|\mathbf{z}_{1:k})}{\tilde{q}(\mathbf{x}_{0:k}|\mathbf{z}_{1:k})},\qquad\mathbf{x}_{0:k}\sim q(\cdot|\mathbf{z}_{1:k})
	\end{equation}

	The variance:
	\begin{equation}
		\mathbb{V}_{q(\cdot|\mathbf{z}_{1:k})}[\tilde{w}_{0:k}]
	\end{equation}
	is non-decreasing in $k$, and equals zero if and only if $q(\mathbf{x}_{0:k}|\mathbf{z}_{1:k})=p(\mathbf{x}_{0:k}|\mathbf{z}_{1:k})$.
\end{proposition}
...
As $k$ increases, the weight distribution typically becomes increasingly concentrated on a small number of particles, eventually leading to severe weight degeneracy. This implies that \eqref{eq:self_normalised_estimator} is a poor approximation of the true PDF. While this is true in most cases, \Cref{th:optimal_proposal} exhibits a proposal density for which the degeneracy problem is reduced.
...
\begin{proposition}[Optimal proposal density]\label{th:optimal_proposal}
	...
	\end{equation}
	the incremental weight variance:
	\begin{equation}
		\mathbb{V}_{q\left(\cdot|x_{k-1}^{(i)},z_k\right)}\left[\tilde{w}_k^{(i)}\right]
	\end{equation}
	is minimised, and the weight degeneracy is therefore delayed. However, it does not remove it in general.
\end{proposition}
...
\begin{proof}
	...
	\begin{equation}
		\begin{aligned}
		\mathbb{V}_{q\left(\cdot|x_{k-1}^{(i)},z_k\right)}\left[\tilde{w}_k^{(i)}\right]\hspace{-5mm}&\hspace{5mm}=\mathbb{E}_{q\left(\cdot|x_{k-1}^{(i)},z_k\right)}\left[{w_k^{(i)}}^2\right]-\mathbb{E}_{q\left(\cdot|x_{k-1}^{(i)},z_k\right)}\left[w_k^{(i)}\right]^2\\
...
While it reduces weight degeneracy, sampling from the posterior requires computing the marginal likelihood:
\begin{equation}
...
\begin{algorithm}[H]
	...
	\State{$\displaystyle\forall i\in[\![1,N]\!],\:w_k^{(i)}=\left.\tilde{w}_k^{(i)}\middle/\sum_{j=1}^N\tilde{w}_k^{(j)}\right.$}\Comment{Normalisation}
	\State{$\left\{{x_k}^{(j)},-,-\right\}_{j=1}^N\gets\operatorname{Resampling}\left(\left\{x_k^{(i)},w_k^{(i)}\right\}_{i=1}^N\right)$}\Comment{Resampling}
	\State{$\displaystyle\hat{x}_k=\frac{1}{N}\sum_{j=1}^Nx_k^{(j)}$}\Comment{State estimation}
	\State{$\displaystyle\hat{P}_k=\frac{1}{N}\sum_{j=1}^N\left(x_k^{(j)}-\hat{x}_k\right)\left(x_k^{(j)}-\hat{x}_k\right)^\top$}\Comment{State covariance}
	\end{algorithmic}\caption{Sampling Importance Resampling}\label{alg:SIR}
\end{algorithm}

\subsubsection{Resampling Algorithms}

Several resampling algorithms can be used in \eqref{eq:resampling}. We present three algorithms, which all select a new particle set from the particle approximation of the PDF:
\begin{equation}
...
\begin{figure}[H]
	\centering\includegraphics[width=0.8\textwidth]{../../Images/Rééchantillonnage CDF.png}
...
\begin{itemize}
	...
	\item\textbf{Kitagawa resampling.} A single draw generates all samples via:
	\begin{equation}
		\left\{\begin{aligned}
...
\begin{remark}[Loss of independence]\label{th:loss_independence}
	Because resampling duplicates particles, the resulting set is no longer independent, and the i.i.d. results of \Cref{th:MC_estimator,th:weighted_MC} no longer apply. However, the resampling is unbiased and the propagation step draws the particles independently conditionally on the past. The Feynman-Kac formalism proves that consistency and asymptotic Gaussianity still hold in this scenario, at the price of a variance larger than in the independent case and which accumulates over time.
\end{remark}
