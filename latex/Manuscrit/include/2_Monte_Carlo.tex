...
Monte Carlo (MC) methods offer an alternative approach that remains consistent under very mild assumptions. Their acccuracy improves asymptotcally with the number of samples, at the expense of an increased computational cost. This section introduces the main concepts for MC-based algorithms to solve the navigation problem \eqref{eq:navigation_problem}.
...
\begin{proposition}[Monte Carlo estimator]\label{th:MC_estimator}
	...
	\end{equation}
	where $\mathbb{V}_\mu[\varphi]\triangleq\left\langle\mu,|\varphi-\langle\mu,\varphi\rangle|^2\right\rangle=\left\langle\mu,|\varphi|^2\right\rangle-|\langle\mu,\varphi\rangle|^2$. Moreover, it is consistent, and its rescaled error converges in distribution to a centred Gaussian:
	\begin{align}
...
\end{equation}
in the sense that, for any measurable function $\phi$:
\begin{equation}
...
It follows from the identity $p(x)=\frac{p(x)}{q(x)}\:q(x)$ and the transfer theorem that:
\begin{equation}
...
\begin{proposition}[Weighted MC estimator]\label{th:weighted_MC}
	...
	\begin{equation}
		\mathbb{E}_q\left[\left|\left\langle S^N(\mu,\nu)-\mu,\varphi\right\rangle\right|^2\right]=\frac{1}{N}\mathbb{V}_\nu\left[\frac{d\mu}{d\nu}\:\varphi\right]
	\end{equation}

	As with the standard estimator, the weighted estimator is consistent and asymptotically Gaussian:
	\begin{align}
...
\begin{proposition}[Self-normalised estimator]\label{th:self_normalised_estimator}
	...
	\end{equation}
	is biased at finite $N$, but consistent under standard integrability and support assumptions:
	\begin{equation}
...
\begin{proof}
	By the strong law of large numbers applied separately to both the numerator and denominator, it comes that:
	\begin{equation}
...
\begin{proposition}[Weight degeneracy]\label{th:variance_weights}
	...
	\end{equation}
	is non-decreasing in $k$, and equals zero if and only if the proposal is the optimal density $q(x_k|x_{k-1},z_k)=p(x_k|x_{k-1},z_k)$.
\end{proposition}
...
\begin{figure}[ht]
	\centering\includegraphics[width=0.9\textwidth]{../../Images/Rééchantillonnage CDF.png}
	\caption{The CDF is defined as the integral of the relevant density function $f$ between $-\infty$ and the current value $x$, \textit{i.e.} $\displaystyle\textup{cdf}:x\mapsto\int_{-\infty}^x{f(t)\:dt}$. In our case, the density is discrete, such that the CDF is piecewise constant, with $\textup{cdf}_0=0$ and $\displaystyle\textup{cdf}_i\triangleq\sum_{j=1}^iw_k^{(j)},\:i\in[\![1,N]\!]$.}\label{fig:CDF}
\end{figure}
