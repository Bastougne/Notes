...
\begin{remark}[Multi-scale kernels]
	It can be easily verified that a linear combination of SPSD kernels with positive weights is itself a SPSD kernel. In particular, for $n\in\mathbb{N}^*$, $\sigma_1^2,\cdots,\sigma_n^2\in\mathbb{R}_+^*$ and $k^{(1)},\cdots,k^{(n)}$ Matérn kernels of correlation lengths $l_1<\cdots<l_n\in\mathbb{R}_+^*$, the multi-scale kernel:
	\begin{equation}
