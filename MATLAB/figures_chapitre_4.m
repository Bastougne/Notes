...
n_cost    = [100, 1600];         % training sizes ADDED to n_train in the cost table.
                                 % n_train itself leads the sweep, so the first column
                                 % is the scenario above and not another experiment;
                                 % 1600 is the 40 x 40 survey of the navigation
                                 % scenarios, and 100 sits between the two to show
                                 % where the r << N regime takes over
d_z_cost  = 1;                   % observed components, scalar magnetometer
...
alpha_band = 0.55;               % opacity of the band, shared with figures_chapitre_3.m
...
latex_cost_table(tab_dir, [n_train, n_cost], d_z_cost, deg_poly);
...
function latex_cost_table(tab_dir, n_sweep, d_z, deg_poly)
% The arithmetic cost of the same five estimators, as a third tabular in the style
% of the two above. Nothing is timed here: these are closed-form flop counts, so
% the table compares what estimating the mean ADDS to the factorisation every
% estimator pays, which is the decomposition the manuscript argues from.
%
% WHY A SWEEP OF n RATHER THAN ONE SURVEY. A single large n would describe an
% experiment the other two tables know nothing about, and the seven points of the
% illustration would then stand for nothing. The training set is therefore the
% column variable, starting at the n_train of the scenario: the first column is
% the example itself, where the mean is NOT cheap, and the later ones show where
% the r << N regime takes over. The table then argues the scaling instead of
% asserting one operating point.
%
% The two simple estimators share one row, their cost being independent of whether
% the mean they were given is right.
    if ~exist(tab_dir, 'dir')
        mkdir(tab_dir);
    end
    N     = n_sweep * d_z;
    setup = N.^3 / 3;                   % Cholesky of G
    query = d_z * N.^2;                 % triangular solves of the error covariance

    names = {'Simple', 'Ordinary', 'Universal, affine', ...
             sprintf('Universal, degree $%d$', deg_poly)};
    r     = [0, 1, 2, deg_poly + 1];    % a degree p basis carries p + 1 monomials
    n     = numel(n_sweep);
    cols  = repmat('c', 1, n);

    fid = fopen(fullfile(tab_dir, 'kriging_cost.tex'), 'w', 'n', 'UTF-8');
    fprintf(fid, '%% Generated by figures_chapitre_4.m, do not edit by hand.\n');
    fprintf(fid, ['%% Closed-form flop counts, not timings, for %d observed component(s)\n' ...
                  '%% and training sets of %s points. Simple kriging estimates no mean, so its\n' ...
                  '%% row is the reconstruction of the field that every estimator pays; the\n' ...
                  '%% others are what estimating the mean adds to it.\n\n'], ...
            d_z, strjoin(arrayfun(@(v) sprintf('%d', v), n_sweep, 'uni', 0), ', '));
    fprintf(fid, '\\begin{tabular}{|l|%s|%s|}\\hline\n', cols, cols);
    ...
    for i = 1:numel(names)
        ...
        if r(i) == 0
            % Simple kriging estimates no mean, so its cost IS the reconstruction
            % of the field that every estimator pays. One row, not two.
            row = [flops_tex(setup, false), flops_tex(query, false)];
            fprintf(fid, ' & %s', row{:});
        else
            s   = 2 * r(i) * d_z * N.^2 + 2 * (r(i) * d_z)^2 * N + (r(i) * d_z)^3 / 3;
            q   = 2 * r(i) * d_z * N;
            row = [flops_tex(s, true), flops_tex(q, true)];
            fprintf(fid, ' & %s', row{:});
        end
        fprintf(fid, '\\\\\\hline\n');
    end
...
function t = flops_tex(v, signed)
% Three significant figures and the bare decimal prefix, so that a column can span
% tens of flops and gigaflops without changing format. The unit itself is in the
% header, six cells carrying "flop" being what pushes the tabular past the text
% block. Signed for the rows that are overheads rather than costs, so that a reader
% cannot mistake one for the other.
    if numel(v) > 1
        t = arrayfun(@(u) flops_tex(u, signed), v, 'UniformOutput', false);
        return
    end
    units = {'', 'k', 'M', 'G', 'T'};
...
% pink and the same opacity as the envelope of figures_chapitre_3.m, so that the
