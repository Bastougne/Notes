...
\begin{proposition}[Simple kriging estimator]\label{th:simple_kriging}
	...
	Here, the term $R$ appears because the quantity being predicted is the noisy query $z=f(x)+\varepsilon$ rather than the latent field value $f(x)$.
\end{proposition}
...
Assuming the mean function to be known is a very strong assumption, that is often not verified in real case scenarios. While an approximated model could be used instead of the real $m$, this would cause the kriging estimator to be over-confident in its estimation. Ordinary kriging (OK) extends its simple counterpart by assuming that $m$ is an unknown constant $m\in\mathbb{R}^{d_z}$, and learning $m$ alongside the weights.
...
\begin{remark}[One update for the three estimators]\label{th:universal_kriging_update}
	\Cref{eq:kriging_update} involves the current estimator and its error covariance, never the Gram matrix. It therefore applies unchanged to ordinary and universal kriging, which are simple kriging under the augmented kernel $k(x,x')+g(x)\:B\:g(x')^\top$ in the limit of a vague prior $B^{-1}\to0$ on the trend coefficients \cite{bib:Rasmussen-regression}. Only the covariance fed to the update changes, from $R_{\mathrm{SK}}$ to $R_{\mathrm{OK}}$ or $R_{\mathrm{UK}}$.
