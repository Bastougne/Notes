...
\begin{equation}
	\hat{x}_k\triangleq\left\langle\tilde{S}^N(\mu_k,\nu_k),\pi_k\right\rangle=\sum_{i=1}^Nw_k^{(i)}\:x_k^{(i)}\approx\mathbb{E}_{p(\cdot|\mathbf{z}_{1:k})}[x_k]\label{eq:PF_MMSE}
\end{equation}
...
\begin{figure}[!htb]
	\centering\includegraphics[width=0.7\textwidth]{../../Images/Rééchantillonnage CDF.pdf}
...
\begin{algorithm}[!htb]
	...
	\State{$\forall i\in[\![1,N]\!],\:\operatorname{cdf}_i=\operatorname{cdf}_{i-1}+w_k^{(i)}$}
	\State{$u_1\sim\mathcal{U}([0,1/N])$}\Comment{Kitagawa samples}
	\State{$\forall j\in[\![2,N]\!],\:u_j=u_1+(j-1)/N$}
	\State{$i=1$}
	\For{$j=1:N$}
