...
\textbf{TODO: à vérifier, sous-section rédigée par Claude}
...
\begin{itemize}
	...
	\begin{equation}
		\mathrm{RMSE}_{\hat{x}}:k\mapsto\sqrt{\frac{1}{n_\mathrm{MC}}\sum_{i=1}^{n_\mathrm{MC}}\left\|\varphi_{\mathrm{pos}}\left(\hat{x}_k^{(i)}\right)-\varphi_{\mathrm{pos}}(x_k)\right\|_2^2}
	\end{equation}
	where $\hat{x}_k^{(i)}$ is the state estimated by the $i$-th filter at time $k$, $x_k$ is the corresponding true state, and $\varphi_{\mathrm{pos}}$ is the projection onto the positions:\textbf{TODO: définir une matrice à la place}
	\begin{equation}
		\varphi_{\mathrm{pos}}:\begin{bmatrix}
		p^{(x)}&p^{(y)}&v^{(x)}&v^{(y)}
