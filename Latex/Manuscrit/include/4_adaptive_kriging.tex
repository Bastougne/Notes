...
The observation function is the scalar ($d_z=1$) OK estimation of the magentic anomaly field, conditioned on the $\tilde{n}$ training points, and degraded by a Gaussian white noise of covariance $R_k=5^2\text{nT}^2$. The chosen kernel is a Matérn kernel with smoothness $\nu=3/2$. \textbf{TODO: dire pk on choisit Matérn à $3/2$.} A bilinear observation function on the same samples is also used as comparison.
...
\begin{figure}[!htb]
	\centering\includegraphics[width=0.7\textwidth]{../../Images/Carte anomalie magnetique.pdf}
	\caption{}\label{fig:magnetic_anomaly_map}
\end{figure}
...
Before analysing the filters themselves, it is important to assess the quality of the reconstructed maps. \Cref{fig:reconstruction_ARMSE} presents the reconstruction ARMSE for ordinary kriging and bilinear interpolation across the different grid resolutions.\textbf{TODO: est-ce que c'est bien normalisé comme \eqref{eq:kriging_ARMSE} ?}

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.6\textwidth]{../../Images/ARMSE de reconstruction.pdf}
	\caption{}\label{fig:reconstruction_ARMSE}\textbf{TODO: mettre des titres sur les figures}
\end{figure}
...
Focusing now on the filter performance, \Cref{fig:kriging_RMSE,fig:kriging_NEES} respectively compare the RMSE and NEES of the filter using the true map, kriging and bilinear interpolation on each of the six grid resolutions.

\begin{figure}[!p]
	\centering
	\includegraphics[width=\textwidth]{../../Images/RMSE par maille.pdf}
	\caption{}\label{fig:kriging_RMSE}
\end{figure}

\begin{figure}[!p]
	\centering
	\includegraphics[width=\textwidth]{../../Images/NEES par maille.pdf}
	\caption{}\label{fig:kriging_NEES}
\end{figure}

Two main results can be drawn from the RMSE curves. First, filters with access to more training points from a denser grid achieve better performance, with the limit case of the filter using the true map outperforming every other configuration. Second, kriging-based filters consistently achieve lower RMSEs than their bilinear counterparts despite having similar map reconstruction accuracy. This result, combined with \Cref{fig:reconstruction_ARMSE}, indicates that the advantage of kriging lies in its ability to quantify reconstruction uncertainty through its covariance matrix.

The NEES curves further highlight the superiority of kriging methods by taking the estimated error covariance matrix into account. While the RMSE ratio between kriging and bilinear interpolation ranges from (\text{this}) to (\text{that}), it increases to (\text{this})--(\text{that}) for the NEES. Thus, kriging not only leads to more accurate state estimates than bilinear interpolation, but also provides a more accurate characterisation of their associated uncertainty.\textbf{TODO: calculer les ratios}
...
\begin{figure}[!htb]
	...
	\caption{}\label{fig:erreur_maille}
\end{figure}

\begin{figure}[!htb]
	\centering\includegraphics[width=\textwidth]{../../Images/Convergence par maille.pdf}
	\caption{}\label{fig:convergence_maille}
\end{figure}

Finally, \Cref{tab:cout_maille} gives the computational cost of each method.
...
\begin{table}[htbp]
	...
	\begin{tabular}{lrrrr}
		...
		\hline
		Ordinary kriging & $99\%$ & $448$ m & $376$ m & $3136$ \\
		Adaptive kriging & $99\%$ & $443$ m & $\mathbf{327}$ m & $\mathbf{53}$ \\
		\hline
	\end{tabular}
	\caption{Adaptive against static kriging at a survey pitch of $3.50$ km, regularised filter, $n_\mathrm{MC}=100$ over ten survey draws.}\label{tab:AK}
\end{table}
...
The remaining parameters were swept at the operating point of this chapter, over
$n_\mathrm{MC}=100$ realisations and five survey draws each. None of $\alpha_W$, $n_{\min}$,
$n_{\max}$ or the size of the subsample used for the local hyperparameter fit moves the
accuracy by more than six per cent, and every one of them moves the cost by a factor of two or
more. \textbf{They are selected at the cheap end of a plateau}, and the ceiling $n_{\max}$ never
binds above two thousand, values of two thousand and six thousand four hundred returning the
same figures.
...
\begin{table}[htbp]
	...
	\begin{tabular}{lrrrr}
		...
		& converged & acquisition & end & $\tilde{n}_\mathrm{w}$ \\
		\hline
		Adaptive kriging & $99\%$ & $443$ m & $327$ m & $53$ \\
		Clustered adaptive kriging & $99\%$ & $459$ m & $325$ m & $48$ \\
		\hline
	\end{tabular}
	\caption{Clustered against plain adaptive kriging at a survey pitch of $3.50$ km, regularised filter, $n_\mathrm{MC}=100$ over ten survey draws.}\label{tab:CAK}
\end{table}
...
\begin{table}[htbp]
	...
	\begin{tabular}{lrrrr}
		...
		\hline
		Adaptive kriging & $1.13$ & $13.27$ & --- & $14.39$ \\
		Clustered adaptive kriging & $0.34$ & $0.62$ & $0.16$ & $\mathbf{1.12}$ \\
		\hline
	\end{tabular}
