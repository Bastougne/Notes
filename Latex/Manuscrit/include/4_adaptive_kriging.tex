...
\begin{equation}
	\mathbb{E}\left[\tilde{N}_{\mathrm{RS},k}^r\right]=p\times\tilde{N}
\end{equation}
...
\begin{algorithm}[!htb]
	...
	\State{Compute $\tilde{\mathbf{x}}_{\mathrm{AK},k}^r$ and $\tilde{\mathbf{z}}_{\mathrm{AK},k}^r$ with \eqref{eq:AK_window} and \eqref{eq:AK_window_z}}\Comment{\textbf{Adaptive subsampling}}
	\State{$\hat{\theta}_k\gets\operatorname{MLE}\left(\tilde{\mathbf{x}}_{\mathrm{AK},k}^r,\tilde{\mathbf{z}}_{\mathrm{AK},k}^r,\hat{\theta}_{k-1}\right)$}\Comment{\textbf{Hyperparameter estimation}}
...
\begin{algorithm}[!htb]
	\begin{algorithmic}
	\State{$\forall i\in[\![1,N]\!],\:x_k^{(i)}\sim p\left(\cdot|x_{k-1}^{(i)}\right)$}\Comment{Particle propagation}
	\State{Compute $\tilde{\mathbf{x}}_{\mathrm{NNAK},k}^r$ and $\tilde{\mathbf{z}}_{\mathrm{NNAK},k}^r$ with \eqref{eq:NNAK_window} and \eqref{eq:NNAK_window_z}}\Comment{\textbf{Adaptive subsampling}}
	\State{$\hat{\theta}_k\gets\operatorname{MLE}\left(\tilde{\mathbf{x}}_{\mathrm{NNAK},k}^r,\tilde{\mathbf{z}}_{\mathrm{NNAK},k}^r,\hat{\theta}_{k-1}\right)$}\Comment{\textbf{Hyperparameter estimation}}
...
The main difference between AK and NNAK lies in the scale at which the particle set is considered. While AK views the particle set as a whole from a macroscopic perspective, NNAK adopts a microscopic point of view by treating each particle individually. In this section, we propose to focus on a mesoscopic viewpoint by clustering the particle set. In this section, we introduce an intermediate, mesoscopic perspective by partitioning the particle set into clusters.

\textbf{TODO: présenter HCA + k-Means + Mean-Shift (+ DBSCAN ? il est bien cet algo en fait)}

\subsection{Clustered Adaptive Kriging}\label{sec:CAK_PF}

In full generality, the number of particle clusters at each time step is unknown prior to the mission, and might vary from one time step to the next. This means that $k$-means cannot be used. Furthermore, the heavy computational cost of HCA prevents it from being used for real-time applications. This is why we used Mean-shift.

AK is the limit for k-means using 1 cluster or Mean-shift with a huge bandwidth, while NNAK is the limit using N clusters or a really small bandwidth.

It must also be linked with the Achille's works on clustered PFs, though we don't use clustering to keep certain modes here.

\textbf{TODO: intro à reprendre évidemment}
