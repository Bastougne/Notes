...
\begin{proposition}[Gaussian conditioning]\label{th:Gaussian_conditioning}
	For $n,p\in\mathbb{N}$, let $U\in\mathbb{R}^n$ and $V\in\mathbb{R}^p$ be jointly Gaussian random variables, of respective means $\mu_U,\mu_V$ and covariance matrices $A\succ0$ and $D\succ0$. There exists $B\in\mathcal{M}_{n,p}(\mathbb{R})$ such that:
	\begin{equation}
	...
	We assume $B$ to be such that this joint covariance is SPD in the sense of \Cref{th:matrix_SPSD}. The conditional distribution of $U$ given $V=v$ is then:
	\begin{equation}
		U|V=v\sim\mathcal{N}\left(\mu_U+BD^{-1}(v-\mu_V),P_{U|V}\right)\label{eq:Gaussian_conditioning}
	\end{equation}
	with:
	\begin{equation}
		P_{U|V}\triangleq A-BD^{-1}B^\top\succ0
	\end{equation}
\end{proposition}

\begin{proof}
	The existence of $B$ is direct by definition of jointly Gaussian random variables. As $D\succ0$ is invertible, $S\triangleq A-BD^{-1}B^\top$ the Schur complement with respect to $D$ is well-defined.

	Let $u\neq0$:
	\begin{equation}
		\begin{bmatrix}u\\-D^{-1}B^\top u\end{bmatrix}^\top\begin{bmatrix}A&B\\B^\top&D\end{bmatrix}\begin{bmatrix}u\\-D^{-1}B^\top u\end{bmatrix}=u^\top\left(A-BD^{-1}B^\top\right)u>0
	\end{equation}
	by SPDness of the joint covariance, the left-hand vector being non-zero. $S$ is thus also invertible, and we can apply the inferior right Schur inversion formula (see \Cref{th:Schur}) to obtain:
	\begin{equation}
		\begin{bmatrix}\bar{A}&\bar{B}\\\bar{C}&\bar{D}\end{bmatrix}\triangleq\begin{bmatrix}A&B\\B^\top&D\end{bmatrix}^{-1}
	\end{equation}

	For some realisation $u$ of $U$ and $v$ of $V$, we define $\tilde{u}\triangleq u-\mu_U$ and $\tilde{v}\triangleq v-\mu_V$, and we compute:
	\begin{equation}
		\begin{aligned}
	...
	and covariance:
	\begin{equation}
		P_{U|V}\triangleq S=A-BD^{-1}B^\top\succ0
	\end{equation}
\end{proof}
...
\begin{proposition}[Schur inversion formula]\label{th:Schur}
	For $n,p\in\mathbb{N}$, let $A\in\mathcal{M}_{n,n}(\mathbb{R})$, $B\in\mathcal{M}_{n,p}(\mathbb{R})$, $C\in\mathcal{M}_{p,n}(\mathbb{R})$, and $D\in\mathcal{M}_{p,p}(\mathbb{R})$, with $A$ invertible. We define the matrix $M$ and its Schur complement with respect to $A$ as:
	\begin{equation}
		M\triangleq\begin{bmatrix}A&B\\C&D\end{bmatrix},\qquad M/A\triangleq D-CA^{-1}B
	\end{equation}

	If $M/A$ is invertible, then $M$ is invertible and:
	\begin{equation}
...
\begin{remark}[Inversion with respect to $D$]
	If $D$ is invertible, one can instead introduce the Schur complement with respect to $D$:
	\begin{equation}
		M/D\triangleq A-BD^{-1}C
	\end{equation}

	When $M/D$ is also invertible, an analogous block inversion formula holds:
	\begin{equation}
...
\begin{proof}
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
	with $I_p+QP$ admitting a right inverse, hence being invertible.
\end{proof}

\begin{proposition}[Generalised Woodbury formula]\label{th:Woodbury}
...
\begin{proof}
	...
	Using \Cref{th:push_through} with $P=A^{-1}B$ and $Q=D^{-1}C$ further gives:
	\begin{equation}
