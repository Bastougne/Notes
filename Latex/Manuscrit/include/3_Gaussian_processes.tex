...
A first way to fill this gap is to reconstruct the field directly from the measurements:
\begin{itemize}
...
\begin{definition}[Gaussian process]\label{th:GP_definition}
	For $d_z\in\mathbb{N}^*$, a stochastic process $(f(x))_{x\in\mathcal{U}}$ taking values in $\mathbb{R}^{d_z}$ is a GP if every finite linear combination of its values follows a Gaussian distribution. Formally, for all $n\in\mathbb{N}^*$, all $x_1,\dots,x_n\in\mathcal{U}$ and all $a_1,\dots,a_n\in\mathbb{R}^{d_z}$, there exist $m\in\mathbb{R}^{d_z}$ and $P\in\mathcal{M}_{d_z,d_z}(\mathbb{R})$ such that $P\succeq0$ and:
	\begin{equation}
...
For the GP to be non-degenerate, every matrix $k(\mathbf{x},\mathbf{x})$ at distinct points must be SPD in the sense of \Cref{th:matrix_SPSD}. \Cref{th:kernel_SPSD} recasts this as a condition on $k$ alone \cite{bib:Schoenberg}, and \Cref{th:kernel_gram_equivalence} proves the equivalence between the two.
...
Since $f$ typically represents a physical field, observations tend to become less correlated as the distance between them increases. \Cref{th:Matérn} derives a common family of radial kernels, called the Matérn kernels \cite{bib:Matérn}, which model this behaviour.
...
\begin{remark}[Multi-scale kernels]
	...
	\end{equation}
	is a valid SPD kernel. When the correlation lengths have different orders of magnitude, such kernels can be used to capture local fluctuations and global trends simultaneously \cite{bib:Rasmussen-covariance}.
\end{remark}
