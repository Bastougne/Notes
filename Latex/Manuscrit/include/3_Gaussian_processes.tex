...
\Cref{fig:Matérn} illustrates the radial decay of Matérn covariance kernels for different smoothness parameters $\nu\in\left\{\frac{1}{2},\frac{3}{2},\frac{5}{2}\right\}$, and their limit as $\nu\to+\infty$. The kernels are evaluated with unit variance $\sigma_f^2=1$ and correlation length $l=1$ to facilitate comparison.

\begin{figure}[!htb]
	\centering\includegraphics[width=0.8\textwidth]{../../Images/Noyaux de Matérn.png}
	\caption{Radial profile of Matérn kernels with increasing smoothness, together with their squared exponential limit. Larger values of $\nu$ produce smoother kernels, with flatter behaviours near the origin and lighter tails.}\label{fig:Matérn}
\end{figure}
...
The main limitation of Matérn kernels is that they require the underlying GP to be scalar, while our observation model takes values in $\mathbb{R}^{d_z}$. To alleviate this issue, \Cref{th:separable_kernel,th:operator_kernel} present two methods to create a vector-valued kernel from a scalar one.

\textbf{TODO: à continuer}
...
\begin{proposition}[Operator-induced kernel]\label{th:operator_kernel}
	
\end{proposition}

\begin{remark}[Multi-scale kernels]\label{th:multi_scale_kernels}
	A linear combination of SPD kernels with positive weights is itself a SPD kernel. In particular, for $n\in\mathbb{N}^*$, $\sigma_1^2,\dots,\sigma_n^2\in\mathbb{R}_+^*$ and $k^{(1)},\dots,k^{(n)}$ Matérn kernels of correlation lengths $l_1<\cdots<l_n\in\mathbb{R}_+^*$, the multi-scale kernel:
	\begin{equation}
		k_{\mathrm{MS}}(r)\triangleq\sum_{i=1}^n\sigma_i^2\:k^{(i)}(r)
	\end{equation}
...
\begin{remark}[Vector-valued kernels]\label{th:vector_valued_kernels}
	The fields of interest take values in $\mathbb{R}^{d_z}$, whereas the Matérn family is scalar. Two constructions turn a scalar kernel $\kappa$ into a matrix-valued one without leaving that family.

	The first weights $\kappa$ by a matrix $B\in\mathcal{S}_{d_z}^{++}(\mathbb{R})$:
	\begin{equation}
		k(x,x')\triangleq B\:\kappa(x,x')\label{eq:separable_kernel}
	\end{equation}
	Writing $B=\sum_m\beta_m\:b_m\:b_m^\top$ turns any quadratic form of $k$ into a positive combination of quadratic forms of $\kappa$, so $k$ is SPD. Such a kernel shares a single correlation structure across the $d_z$ components, which $B$ can only rescale and mix.

	The second transports a scalar GP through a linear operator. If $g\sim\mathcal{GP}(0,\kappa)$ and $L$ is linear, then $L\:g$ is a GP of kernel:
	\begin{equation}
		k(x,x')\triangleq L_x\:L_{x'}^\top\kappa(x,x')\label{eq:operator_kernel}
	\end{equation}
	which is a covariance by construction, hence SPSD. Unlike \eqref{eq:separable_kernel}, it correlates the components differently at every pair of points, and can therefore encode a physical constraint. A magnetic anomaly field derives from a scalar potential, so $L=-\nabla$ yields a kernel enforcing $\nabla\times f=0$ at every point rather than learning it from the data \cite{bib:magnetic-GP}. The construction differentiates $\kappa$ once in each argument, and thus requires $\nu>1$, which rules out the exponential kernel.
\end{remark}
