...
\begin{definition}[Gaussian process]\label{th:GP_definition}
	For $d_z\in\mathbb{N}^*$, a stochastic process $(f(x))_{x\in\mathcal{U}}$ taking values in $\mathbb{R}^{d_z}$ is a GP if every finite linear combination of its values follows a Gaussian distribution. Formally, for all $n\in\mathbb{N}^*$, all $x_1,\dots,x_n\in\mathcal{U}$ and all $a_1,\dots,a_n\in\mathbb{R}^{d_z}$, there exist $\mu\in\mathbb{R}^{d_z}$ and $\Sigma\in\mathcal{S}_{d_z}^+(\mathbb{R})$ such that:
	\begin{equation}
		\sum_{i=1}^na_i^\top f(x_i)\sim\mathcal{N}(\mu,\Sigma)
	\end{equation}

	The GP is said to be non-degenerate if $\Sigma\succ0$ whenever the $x_1,\dots,x_n$ are pairwise distinct and the coefficients $a_1,\dots,a_n$ are not all zero.
\end{definition}
...
\begin{definition}[Radial kernel]\label{th:radial_kernel}
	...
	\begin{equation}
		\forall x,x'\in\mathbb{R}^{d_x},\:k(x,x')=g(\|x-x'\|)
	\end{equation}

	To simplify notations, we identify $g$ with $k$ in the remainder of this document, such that we write:
	\begin{equation}
...
\begin{equation}
	p(Z|X,\theta)=\frac{1}{\sqrt{(2\pi)^n\:\det(K_\theta)}}\:\exp\left(-\frac{1}{2}Z^\top K_\theta^{-1}Z\right)\label{eq:marginal_likelihood}
\end{equation}
