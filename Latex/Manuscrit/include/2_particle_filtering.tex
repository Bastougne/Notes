...
\begin{itemize}
	...
	\item It also replicates the heaviest particles and discards the lightest, so the set progressively concentrates on a few distinct locations. This loss of diversity, known as particle impoverishment, is especially severe in low evolution noise or high-dimensional settings, where little diversity is re-introduced at the propagation step. It is illustrated by \Cref{fig:impoverishment}.
\end{itemize}

\begin{figure}[H]
	\centering\includegraphics[width=0.6\textwidth]{../../Images/Appauvrissement particulaire.png}
...
To limit both issues, the generic particle filter (PF) resamples only when the weight distribution has degraded significantly. This degradation is measured by the effective sample size:
\begin{equation}
	N_{eff}\triangleq\frac{N}{1+\mathbb{V}_{q(\cdot|\mathbf{z}_{1:k})}\left[\dfrac{p(\mathbf{x}_{0:k}|\mathbf{z}_{1:k})}{q(\mathbf{x}_{0:k}|\mathbf{z}_{1:k})}\right]}
\end{equation}

Its denominator increases with the variance of the importance weights, whose growth over time corresponds to the weight degeneracy characterised by \Cref{th:variance_weights}. When all the weights are uniform, the variance is zero, and thus $N_{eff}=N$. Conversely, the more unevenly they are distributed, the smaller $N_{eff}$, with $N_{eff}\to1$ when the mass concentrates on a single particle.

Since the true importance weights are unknown, $N_{eff}$ is estimated by replacing $\mathbb{V}_{q(\cdot|\mathbf{z}_{1:k})}[\cdot]$ with the empirical spread of the normalised weights, which gives:
\begin{equation}
	\hat{N}_{eff}\triangleq\frac{1}{\displaystyle\sum_{i=1}^N\left(w_k^{(i)}\right)^2}\leq N
\end{equation}

Like $N_{eff}$, this quantity ranges from $N$ for uniform weights, down to $1$ in the fully degenerate case. The PF then performs the resampling step only when this estimate falls below a threshold:
\begin{equation}
	\hat{N}_{eff}<N_{th}
\end{equation}
where $N_{th}$ is a predetermined value, typically chosen proportional to $N$.
...
\begin{algorithm}[H]
	...
	\EndIf
	\State{$\displaystyle\hat{x}_k=\sum_{j=1}^Nw_k^{(j)}x_k^{(j)}$}\Comment{State estimation}
	\State{$\displaystyle\hat{P}_k=\sum_{j=1}^Nw_k^{(j)}\left(x_k^{(j)}-\hat{x}_k\right)\left(x_k^{(j)}-\hat{x}_k\right)^\top$}\Comment{State covariance}
...
Since this measure is singular, resampling from it can only reproduce the existing particles. The regularised particle filter (RPF) addresses this issue through kernel density estimation, by replacing each Dirac with a smooth kernel to obtain the absolutely continuous measure:
\begin{equation}
	p(x_k|\mathbf{z}_{1:k})\:dx_k\approx\sum_{i=1}^Nw_k^{(i)}\:K_h\left(x_k-x_k^{(i)}\right)dx_k\label{eq:RPF_kernel}
\end{equation}

New particles are then drawn from this continuous density rather than from the discrete set, which restores diversity, as illustrated by \Cref{fig:regularisation} and formalised by \Cref{th:regularisation_kernel,th:KDE}.

\begin{figure}[H]
	\centering\includegraphics[width=0.8\textwidth]{../../Images/Régularisation par noyau.png}
...
\begin{itemize}
	\item\textbf{Epanechnikov kernel.} When the prior $p$ is Gaussian, and the covariance matrix is equal to the identity, this kernel is optimal in the sense of the mean integrated squared error (MISE). It is defined by:
	\begin{equation}
		\left\{\begin{aligned}
...
It must be noted that the assumptions for the Epanechnikov, and consequently the Gaussian, regularisation are not usually met in practice. Indeed, the estimated covariance matrix $\hat{P}_k$ is not always equal to $I$. To solve this issue, a whitening step is performed before the regularisation, during which every particle is replaced by $A_k^{-1}\:x_k^{(i)}$, where $A_k$ is the Cholesky factor of $\hat{P}_k$:
\begin{equation}
	\hat{P}_k=A_k\:A_k^\top
\end{equation}
...
\begin{algorithm}[H]
	...
	\EndIf
	\State{$\displaystyle\hat{x}_k\gets\sum_{j=1}^Nw_k^{(j)}x_k^{(j)}$}\Comment{State estimation}
	\State{$\displaystyle\hat{P}_k\gets\sum_{j=1}^Nw_k^{(j)}\left(x_k^{(j)}-\hat{x}_k\right)\left(x_k^{(j)}-\hat{x}_k\right)^\top$}\Comment{State covariance}
...
\begin{itemize}
	...
	\end{align}
	\item\textbf{State estimation.} Finally, the full state is estimated, its covariance combining the particle dispersion with the Kalman covariance of each linear substate:
	\begin{align}
		x_k^{(i)}&\gets\left[x_k^{n(i)},\hat{x}_{k|k}^{l(i)}\right]\label{eq:RBPF_estimation_debut}\\
		\hat{x}_k&=\sum_{i=1}^Nw_k^{n(i)}\:x_k^{(i)}\\
		\hat{P}_k&=\sum_{i=1}^Nw_k^{n(i)}\left(x_k^{(i)}-\hat{x}_k\right)\left(x_k^{(i)}-\hat{x}_k\right)^\top+\sum_{i=1}^Nw_k^{n(i)}\:\begin{bmatrix}0&0\\0&P_{k|k}^{l(i)}\end{bmatrix}\label{eq:RBPF_estimation_fin}
	\end{align}
\end{itemize}
...
\begin{algorithm}[H]
	...
	\State{$\forall i\in[\![1,N]\!]$, estimate $\hat{x}_k$ and $\hat{P}_k$ \hfill using \eqref{eq:RBPF_estimation_debut} through \eqref{eq:RBPF_estimation_fin}}
	\end{algorithmic}\caption{Rao-Blackwellised Particle Filter}\label{alg:RBPF}
\end{algorithm}

It must be noted that the version presented by \Cref{alg:RBPF} uses a simple SIS to handle the particles, but any more advanced algorithm presented in this section can be used instead. Furthermore, the likelihood $\tilde{p}\left(z_k|x_k^{n(i)}\right)$ and the transition $\tilde{p}\left(x_k^{n(i)}|x_{k-1}^{n(i)}\right)$ in the weight update are white Gaussian densities, with covariances $S_k^{l(i)}$ and $S_k^{n(i)}$ respectively. This is why $S_k^{l(i)}$ is computed within the Kalman prediction step, before the weight update. However, the Kalman correction step itself must be performed after the weight update in situations where a resampling step is involved.
...
\textbf{TODO: conclure/introduire la partie suivante ?}
