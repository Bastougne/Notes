...
\begin{figure}[!htb]
	\centering\includegraphics[width=0.9\textwidth]{../../Images/Appauvrissement particulaire.pdf}
...
To limit both issues, the generic particle filter (PF) \cite{bib:PF-Sarkka} resamples only when the weight distribution has degraded significantly. This degradation is measured by the effective sample size \cite{bib:sequential-imputations}:
\begin{equation}
...
Since this measure is singular, resampling from it can only reproduce the existing particles. The regularised particle filter (RPF) \cite{bib:RPF} addresses this issue through kernel density estimation \cite{bib:Rosenblatt,bib:Parzen,bib:KDE-multivariate}, by replacing each Dirac with a smooth kernel to obtain the absolutely continuous measure:
\begin{equation}
	p(x_k|\mathbf{z}_{1:k})\:dx_k\approx\sum_{i=1}^Nw_k^{(i)}\:K_h\left(x_k-x_k^{(i)}\right)dx_k\label{eq:RPF_kernel}
\end{equation}

New particles are then drawn from this continuous density rather than from the discrete set, which restores diversity. This is illustrated by \Cref{fig:regularisation} and formalised by \Cref{th:regularisation_kernel,th:KDE}. Being absolutely continuous, this approximation also supplies the density that \eqref{eq:RPF_dirac} lacks, and thus makes the MAP estimator of \cref{sec:state_estimation} well-defined at the cost of a numerical maximisation.

\begin{figure}[!htb]
	\centering\includegraphics[width=0.8\textwidth]{../../Images/Régularisation par noyau.pdf}
...
\begin{definition}[Regularisation kernel]\label{th:regularisation_kernel}
...
\begin{definition}[Regularised measure]
	...
	In particular, the regularised self-normalised measure verifies:
	\begin{equation}
		\tilde{S}_{K_h}^N(\mu,\nu)=\sum_{i=1}^Nw^{(i)}\:\left(K_h*\delta_{X^{(i)}}\right)
	\end{equation}
\end{definition}

\begin{proposition}[Regularised estimator]\label{th:KDE}
	The regularised self-normalised estimator is:
	\begin{equation}
		\left\langle\tilde{S}_{K_h}^N(\mu,\nu),\varphi\right\rangle=\sum_{i=1}^Nw^{(i)}\:\left(K_h*\varphi\right)\left(X^{(i)}\right)=\left\langle\tilde{S}^N(\mu,\nu),K_h*\varphi\right\rangle
	\end{equation}

	It is a biased and inconsistent estimator in general:
	\begin{equation}
		\left\langle\tilde{S}_{K_h}^N(\mu,\nu),\varphi\right\rangle\overset{a.s.}{\longrightarrow}\langle\mu,K_h*\varphi\rangle
	\end{equation}

	However, it becomes consistent when $\varphi$ is linear. In particular:
	\begin{equation}
		\left\langle\tilde{S}_{K_h}^N(\mu_k,\nu_k),\pi_k\right\rangle\overset{a.s.}{\longrightarrow}\langle\mu_k,\pi_k\rangle=\mathbb{E}_{p(\cdot|\mathbf{z}_{1:k})}[x_k]
	\end{equation}
\end{proposition}

\begin{proof}
	Let $i\in[\![1,N]\!]$. By definition of $*$, we have:
	\begin{equation}
		(K_h*\delta_{X^{(i)}})(dx)=K_h(x-X^{(i)})\:dx
	\end{equation}
	and thus, by symmetry of $K_h$:
	\begin{equation}
		\begin{aligned}
		\langle K_h*\delta_{X^{(i)}},\varphi\rangle&=\int{\varphi(x)K_h(x-X^{(i)})\:dx}\\
		&=\int{\varphi(x)K_h(X^{(i)}-x)\:dx}\\
		&=(K_h*\varphi)(X^{(i)})
		\end{aligned}
	\end{equation}

	In turn, this result gives:
	\begin{equation}
		\left\langle\tilde{S}_{K_h}^N(\mu,\nu),\varphi\right\rangle=\sum_{i=1}^Nw^{(i)}\:(K_h*\varphi)\left(X^{(i)}\right)=\left\langle\tilde{S}^N(\mu,\nu),K_h*\varphi\right\rangle
	\end{equation}
	and \Cref{th:self_normalised_estimator}, applied to $K_h*\varphi$, yields the a.s. limit $\langle\mu,K_h*\varphi\rangle$.

	Assume that $\varphi$ is linear, that is $\varphi:x\mapsto A\:x$ for some $A\in\mathcal{M}_{d_{\varphi},d_x}(\mathbb{R})$. Then, by normalisation and symmetry of $K_h$, for all $x\in\mathbb{R}^{d_x}$:
	\begin{equation}
		\begin{aligned}
		(K_h*\varphi)(x)&=\int{K_h(y)\varphi(x-y)\:dy}\\
		&=\varphi(x)\int{K_h(y)\:dy}-A\int{yK_h(y)\:dy}\\
		&=\varphi(x)
		\end{aligned}
	\end{equation}
	Hence $K_h*\varphi=\varphi$, and the limit reduces to $\langle\mu,\varphi\rangle$. In particular, for the linear projection $\pi_k$, it becomes $\langle\mu_k,\pi_k\rangle=\mathbb{E}_{p(\cdot|\mathbf{z}_{1:k})}[x_k]$.
\end{proof}
...
\begin{itemize}
	\item\textbf{Epanechnikov kernel.} This kernel minimises the mean integrated squared error (MISE) among all regularisation kernels \cite{bib:Epanechnikov}, and the associated bandwidth is optimal when the density is Gaussian with identity covariance. It is defined by:
	\begin{equation}
		\left\{\begin{aligned}
...
On the other hand, the unit covariance is a scaling requirement, and can be enforced exactly by a whitening step before the regularisation. This amounts to replacing every particle by $A_k^{-1}\:x_k^{(i)}$, where $A_k$ is a Cholesky factor of $\hat{P}_k$:
\begin{equation}
	\hat{P}_k=A_k\:A_k^\top
\end{equation}
...
\begin{itemize}
	...
	\end{equation}
	\item Since the parent index uniquely determines the ancestor trajectory, conditioning on $i$ is equivalent to conditioning on $\mathbf{x}_{0:k-1}^{(i)}$. Moreover, \Cref{th:Markov} establishes that $(x_k)_{k\geq0}$ is a Markov chain, and thus:
	\begin{equation}
...
\begin{remark}[Comparison between the RPF and the APF]\label{th:RPF_APF}
...
\begin{equation}
	...
	\end{bmatrix}\right),\qquad n_k\sim\mathcal{N}(0,R_k)
\end{equation}
...
\begin{equation}
	p(x_k|\mathbf{z}_{1:k})\:dx_k\approx\sum_{i=1}^Nw_k^{n(i)}\:\delta_{x_k^{n(i)}}\left(dx_k^n\right)\:\mathcal{N}\left(\hat{x}_{k|k}^{l(i)},P_{k|k}^{l(i)}\right)\left(dx_k^l\right)
\end{equation}
...
\begin{algorithm}[!htb]
	...
	\State{$\forall i\in[\![1,N]\!]$, propagate $x_k^{n(i)}$ \hfill using \eqref{eq:RBPF_propagation}}\phantom{ through \eqref{eq:RBPF_precorrection_fin}\hspace*{16mm}\mbox{}}
	\State{$\forall i\in[\![1,N]\!]$, update $\hat{x}_{k-1|k-1}^{l(i)}$ and $P_{k-1|k-1}^{l(i)}$ \hfill using \eqref{eq:RBPF_precorrection_debut} through \eqref{eq:RBPF_precorrection_fin}\hspace*{16mm}\mbox{}}
	\State{$\forall i\in[\![1,N]\!]$, predict $\hat{x}_{k|k-1}^{l(i)}$ and $P_{k|k-1}^{l(i)}$ \hfill using \eqref{eq:RBPF_prediction_debut} through \eqref{eq:RBPF_prediction_fin}\hspace*{16mm}\mbox{}}
	\State{$\forall i\in[\![1,N]\!]$, update $w_k^{n(i)}$ \hfill using \eqref{eq:RBPF_update_debut} through \eqref{eq:RBPF_update_fin}\hspace*{16mm}\mbox{}}
	\State{$\forall i\in[\![1,N]\!]$, compute $\hat{x}_{k|k}^{l(i)}$ and $P_{k|k}^{l(i)}$ \hfill using \eqref{eq:RBPF_correction_debut} through \eqref{eq:RBPF_correction_fin}\hspace*{16mm}\mbox{}}
